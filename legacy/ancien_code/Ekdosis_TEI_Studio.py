...
def charger_configuration():
    ...
    # Source canonique: "Numéro du témoin de référence"
    # Fallback legacy: "index_ref" pour compatibilité avec d'anciens fichiers.
    index_ref_brut = config.get("Numéro du témoin de référence", config.get("index_ref", 0))
    try:
        index_ref = int(index_ref_brut)
    except (TypeError, ValueError):
        index_ref = 0
    if index_ref < 0:
        index_ref = 0
    config["Numéro du témoin de référence"] = index_ref
...
def editer_config_apres_chargement(config):
    ...
    #DEBUG
    print("DEBUG - index ref", config.get("Numéro du témoin de référence", 0))
...
def pretraiter_lignes_generation(texte):
    lignes = texte.splitlines()
    return [convertir_italique_tei(l) for l in lignes]


def construire_blocs_acte_scene_persos(
        bloc_acte,
        bloc_scene,
        bloc_persos,
        nombre_temoins_predefini,
        ref_index,
        numero_acte,
        numero_scene
):
    ...
    return {
        "tei_head": tei_head,
        "ekddiv_acte": ekddiv_acte,
        "ekdosis_head": ekdosis_head,
        "tei_scene_head": tei_scene_head,
        "ekddiv_scene": ekddiv_scene,
        "ekdosis_scene_head": ekdosis_scene_head,
        "tei_persos_stage": tei_persos_stage,
        "ekdosis_persos_stage": ekdosis_persos_stage
    }


def construire_header_tei(titre, auteur, editeur, num_acte, num_scene):
...
def fermer_bloc_speaker_standard(resultat_tei, resultat_ekdosis):
    resultat_tei.append("    </sp>")
    resultat_ekdosis.append("      \\end{ekdverse}\n    \\end{speech}")


def ouvrir_bloc_speaker_standard(resultat_tei, resultat_ekdosis, speakers, ref_index):
...
def traiter_didascalies_explicites(lignes, resultat_tei, resultat_ekdosis):
    if not (all(l.startswith('**') for l in lignes) and len(lignes)):
        return False
...
def traiter_variantes_vers_entier(lignes, vers_courant, resultat_tei, resultat_ekdosis):
    if not (all(l.startswith('#####') for l in lignes) and len(lignes)):
        return False, vers_courant
...
def comparer_etats():
    ...
    implicit_counter = 0
    texte = zone_saisie.get("1.0", tk.END).strip()
    lignes = pretraiter_lignes_generation(texte)
    ...
    ref_index = liste_ref.current()
    blocs_structure = construire_blocs_acte_scene_persos(
        bloc_acte,
        bloc_scene,
        bloc_persos,
        nombre_temoins_predefini,
    ...
    tei_head = blocs_structure["tei_head"]
    ekddiv_acte = blocs_structure["ekddiv_acte"]
    ekdosis_head = blocs_structure["ekdosis_head"]
    tei_scene_head = blocs_structure["tei_scene_head"]
    ekddiv_scene = blocs_structure["ekddiv_scene"]
    ekdosis_scene_head = blocs_structure["ekdosis_scene_head"]
    tei_persos_stage = blocs_structure["tei_persos_stage"]
    ekdosis_persos_stage = blocs_structure["ekdosis_persos_stage"]
    ...
    resultat_ekdosis = []
    resultat_tei = construire_header_tei(titre, auteur, editeur, num_acte, num_scene)
    ...
    for speaker, bloc in dialogues:
        ...
        if speakers != dernier_locuteur:
            if dernier_locuteur is not None:
                fermer_bloc_speaker_standard(resultat_tei, resultat_ekdosis)

            ref_index = liste_ref.current()
            ouvrir_bloc_speaker_standard(resultat_tei, resultat_ekdosis, speakers, ref_index)
        ...
        for sous_bloc in sous_blocs:
            ...
            if traiter_didascalies_explicites(lignes, resultat_tei, resultat_ekdosis):
                continue
            ...
            did_process_full_line_variants, vers_courant = traiter_variantes_vers_entier(
                lignes, vers_courant, resultat_tei, resultat_ekdosis
            )
            if did_process_full_line_variants:
                continue
