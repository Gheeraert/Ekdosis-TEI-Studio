\ekddiv{type=act, n=1, depth=2}
\stage{ACTE 1}
\ekddiv{type=scene, n=1, depth=3}
\stage{SCENE III.}
\stage{\app{\lem[wit={A, B}]{IOCASTE,}\rdg[wit={C, D, E}]{JOCASTE,}} ANTIGONE, ETEOCLE, OLYMPE.}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C, D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{1}{OLympe, \app{\lem[wit={A, E}]{soustiens~moy,}\rdg[wit={B}]{soûtiens-moy,}\rdg[wit={C, D}]{soûtiens-moi,}} ma douleur est \app{\lem[wit={A, B, E}]{extréme.}\rdg[wit={C, D}]{extrême.}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, C, D, E}]{ETEOCLE.}\rdg[wit={B}]{ETEOCLE}}}
  \begin{ekdverse}
    \vnum{2}{\app{\lem[wit={A}]{Madame qu’auez~vous~? Et quel mal si caché…}\rdg[wit={B}]{Madame, qu’avez-vous~? Et quel mal si caché…}\rdg[wit={C}]{Madame, qu’avez-vous~? \& quel mal si caché…}\rdg[wit={D}]{Madame, qu’avez-vous~? \& quel trouble…}\rdg[wit={E}]{Madame qu’avez~vous~? Et quel mal si caché….}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C, D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{3}{\app{\lem[wit={A, E}]{Ah~! mon Fils, de quel sang estes vous-là taché~?}\rdg[wit={B, C}]{Ah~! mon Fils, de quel sang revenez-vous taché~?}\rdg[wit={D}]{~~~~~~~~~~~~~~~~~~Ah~! mon Fils,}}\\}
    \vnum{4}{\app{\lem[wit={A, B, C, D}]{(lacune)}\rdg[wit={E}]{Quelles traces de sang voi-je sur vos habits~?}}\\}
    \vnum{5}{\app{\lem[wit={A, E}]{Est-ce~de~vostre~Frere,}\rdg[wit={B, C, D}]{Est-ce~du~sang~d’un~Frere,}} ou n’est-ce point du vostre~?\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{ETEOCLE.}
  \begin{ekdverse}
    \vnum{6}{Non, Madame, ce n’est \app{\lem[wit={A, E}]{ny}\rdg[wit={B, C, D}]{ni}} de \app{\lem[wit={A}]{l’vn}\rdg[wit={B, C, D, E}]{l’un}} \app{\lem[wit={A, E}]{ny}\rdg[wit={B, C, D}]{ni}} de \app{\lem[wit={A, E}]{l’autre,}\rdg[wit={B, C, D}]{l’autre.}}\\}
    \vnum{7}{\app{\lem[wit={A, B, C, E}]{Polinice à mes yeux ne s’est point presenté,}\rdg[wit={D}]{Dans son camp jusqu’icy Polinice arresté,}}\\}
    \vnum{8}{\app{\lem[wit={A}]{Et l’on s’est peu battu d’vn \& d’autre costé.}\rdg[wit={B, C}]{Et l’on s’est peu battu d’un \& d’autre costé,}\rdg[wit={D}]{Pour combattre à mes yeux ne s’est point presenté.}\rdg[wit={E}]{Et l’on s’est peu battu d’un \& d’autre costé.}}\\}
    \vnum{9}{\app{\lem[wit={A}]{Seulement quelques Grecs d’vn insolent courage,}\rdg[wit={B}]{Du Camp des Argiens une troupe hardie,}\rdg[wit={C}]{Du Camp des Argiens une troupe hardie}\rdg[wit={D}]{D’Argiens seulement une troupe hardie}\rdg[wit={E}]{Seulement quelques Grecs d’un insolent courage,}}\\}
    \vnum{10}{\app{\lem[wit={A, E}]{M’ayant osé d’abord disputer le passage,}\rdg[wit={B, C, D}]{M’a voulu de nos murs disputer la sortie.}}\\}
    \vnum{11}{\app{\lem[wit={A}]{I’ay}\rdg[wit={B, C, D, E}]{J’ay}} fait mordre la poudre à ces audacieux,\\}
    \vnum{12}{Et leur sang est celuy qui paroist à vos yeux.\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C, D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{13}{\app{\lem[wit={A}]{Mais pourquoy donc sortir auecque vostre Armée,}\rdg[wit={B}]{Mais pourquoy donc sortir auecque vostre armée~?}\rdg[wit={C}]{Mais pourquoi donc sortir avecque vostre armée~?}\rdg[wit={D}]{Mais que prétendiez vous~? \& quelle ardeur soudaine}\rdg[wit={E}]{Mais pourquoy donc sortir avecque vostre Armée,}}\\}
    \vnum{14}{\app{\lem[wit={A}]{Quel est ce mouuement qui m’a tant alarmée~?}\rdg[wit={B, C, E}]{Quel est ce mouvement qui m’a tant alarmée~?}\rdg[wit={D}]{Vous a fait tout à coup descendre dans la plaine~?}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{ETEOCLE.}
  \begin{ekdverse}
    \vnum{15}{\app{\lem[wit={A, E}]{Madame}\rdg[wit={B, C, D}]{Madame,}} il estoit temps que j’en \app{\lem[wit={A}]{vsasse}\rdg[wit={B, C, D, E}]{usasse}} \app{\lem[wit={A, C, E}]{ainsi,}\rdg[wit={B, D}]{ainsy,}}\\}
    \vnum{16}{Et \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} perdois ma gloire à demeurer \app{\lem[wit={A, B, D, E}]{icy.}\rdg[wit={C}]{ici.}}\\}
    \vnum{17}{\app{\lem[wit={A}]{Ie n’ay que trop languy derriere vne muraille,}\rdg[wit={B, C}]{Je n’ay que trop langui derriere une muraille,}\rdg[wit={D}]{(lacune)}\rdg[wit={E}]{Je n’ay que trop languy derriere une muraille,}}\\}
    \vnum{18}{\app{\lem[wit={A}]{Ie brûlois de me voir en vn champ de bataille,}\rdg[wit={B, C}]{Je brûlois de me voir en un champ de bataille.}\rdg[wit={D}]{(lacune)}\rdg[wit={E}]{Je brûlois de me voir en un champ de bataille,}}\\}
    \vnum{19}{\app{\lem[wit={A, B, C, E}]{Lors~que l’on peut paroistre au milieu des hazards,}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{20}{\app{\lem[wit={A}]{Vn grand cœur est honteux de garder des remparts.}\rdg[wit={B, C}]{Un grand cœur est honteux de garder des rempars.}\rdg[wit={D}]{(lacune)}\rdg[wit={E}]{Un grand cœur est honteux de garder des remparts.}}\\}
    \vnum{21}{\app{\lem[wit={A}]{I’estois las d’endurer que le fier Polinice}\rdg[wit={B, C, E}]{J’estois las d’endurer que le fier Polinice}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{22}{\app{\lem[wit={A, B, C, E}]{Me reprochast tout haut cét indigne exercice,}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{23}{\app{\lem[wit={A, E}]{Et criast aux Thebains, afin de les gaigner,}\rdg[wit={B, C}]{Et criast aux Thebains, afin de les gagner,}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{24}{\app{\lem[wit={A, B}]{Que ie laissois aux fers ceux qui me font regner.}\rdg[wit={C}]{Que je laissois perir ceux qui me font regner.}\rdg[wit={D}]{(lacune)}\rdg[wit={E}]{Que je laissois aux fers ceux qui me font regner.}}\\}
    \vnum{25}{Le Peuple à qui la faim se faisoit déja craindre,\\}
    \vnum{26}{De mon peu de vigueur commençoit à se plaindre,\\}
    \vnum{27}{Me reprochant déja qu’il \app{\lem[wit={A, E}]{m’auoit}\rdg[wit={B, C, D}]{m’avoit}} couronné,\\}
    \vnum{28}{Et que j’occupois mal le rang qu’il m’a donné.\\}
    \vnum{29}{Il le faut satisfaire, \& \app{\lem[wit={A, D, E}]{quoy~qu’il}\rdg[wit={B, C}]{quoy-qu’il}} en \app{\lem[wit={A}]{arriue,}\rdg[wit={B, C, D, E}]{arrive,}}\\}
    \vnum{30}{Thebes dés \app{\lem[wit={A, B, D, E}]{aujourd’huy}\rdg[wit={C}]{aujourd’hui}} ne sera plus \app{\lem[wit={A}]{captiue,}\rdg[wit={B, C, D, E}]{captive,}}\\}
    \vnum{31}{\app{\lem[wit={A}]{Ie}\rdg[wit={B, C, D, E}]{Je}} veux, en n’y laissant aucun de mes soldats,\\}
    \vnum{32}{Qu’elle soit seulement juge de nos \app{\lem[wit={A, B, E}]{combats.}\rdg[wit={C}]{combats,}\rdg[wit={D}]{combats~:}}\\}
    \vnum{33}{\app{\lem[wit={A}]{I’ay}\rdg[wit={B, C, D, E}]{J’ay}} des forces assez pour tenir la \app{\lem[wit={A, C, D, E}]{campagne,}\rdg[wit={B}]{camgagne,}}\\}
    \vnum{34}{Et si quelque \app{\lem[wit={A, B, C, E}]{bon-heur}\rdg[wit={D}]{bonheur}} nos armes accompagne,\\}
    \vnum{35}{L’insolent Polinice \& ses \app{\lem[wit={A, E}]{Grecs}\rdg[wit={B, C, D}]{fiers}} \app{\lem[wit={A, E}]{orgüeilleux,}\rdg[wit={B}]{Alliez,}\rdg[wit={C, D}]{Alliez}}\\}
    \vnum{36}{Laisseront Thebes libre, ou \app{\lem[wit={A, E}]{mouront}\rdg[wit={B, C, D}]{mourront}} à \app{\lem[wit={A, E}]{ses}\rdg[wit={B, C, D}]{mes}} \app{\lem[wit={A, E}]{yeux.}\rdg[wit={B, C, D}]{piez.}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C}]{JOCASTE}\rdg[wit={D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{37}{\app{\lem[wit={A}]{Vous preserue le Ciel d’vne telle Victoire,}\rdg[wit={B}]{Vous preserve le Ciel d’une telle Victoire.}\rdg[wit={C}]{Vous preserve le Ciel d’une telle victoire.}\rdg[wit={D}]{(lacune)}\rdg[wit={E}]{Vous preserve le Ciel d’une telle Victoire,}}\\}
    \vnum{38}{\app{\lem[wit={A}]{Thebes ne veut point voir vne action si noire,}\rdg[wit={B, C, E}]{Thebes ne veut point voir une action si noire,}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{39}{\app{\lem[wit={A, E}]{Laissez là son salut \& n’y songez jamais~;}\rdg[wit={B, C}]{Laissez-là son salut \& n’y songez jamais~;}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{40}{\app{\lem[wit={A, B, E}]{La Guerre vaut bien mieux que cette affreuse Paix.}\rdg[wit={C}]{La Guerre vaut bien-mieux que cette affreuse Paix.}\rdg[wit={D}]{(lacune)}}\\}
    \vnum{41}{\app{\lem[wit={A}]{Dure-t’elle à iamais cette cruelle Guerre,}\rdg[wit={B}]{Dure-t’elle à jamais cette cruelle Guerre,}\rdg[wit={C}]{(lacune)}\rdg[wit={D, E}]{Dont le flambeau fatal desole cette terre.}}\\}
    \vnum{42}{\app{\lem[wit={A}]{Prolongez nos mal-heurs, augmentez-les tousiours,}\rdg[wit={B}]{Prolongez nos mal-heurs, augmentez-les tousjours,}\rdg[wit={C}]{(lacune)}\rdg[wit={D}]{Plustost qu’vn si grand crime en arreste le cours.}\rdg[wit={E}]{Plustost qu’un si grand crime en arreste le cours.}}\\}
    \vnum{43}{\app{\lem[wit={A}]{Vous-mesme d’vn tel sang soüilleriez vous vos Armes~?}\rdg[wit={B}]{Vous-mesme d’un tel sang soüilleriez-vous vos Armes~?}\rdg[wit={C}]{Vous-mesme d’un tel sang, soüillerez-vous vos Armes~?}\rdg[wit={D}]{Vous pourriez d’un tel sang, ô Ciel~! souiller vos armes~?}\rdg[wit={E}]{Vous-mesme d’un tel sang soüilleriez vous vos Armes~?}}\\}
    \vnum{44}{La \app{\lem[wit={A, E}]{Courone}\rdg[wit={B, C, D}]{Couronne}} pour vous \app{\lem[wit={A, E}]{a~t’elle}\rdg[wit={B, C}]{a-t’elle}\rdg[wit={D}]{a-t-elle}} tant de charmes~?\\}
    \vnum{45}{Si par \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} parricide il la falloit \app{\lem[wit={A, E}]{gaigner}\rdg[wit={B, C, D}]{gagner,}}\\}
    \vnum{46}{Ah~! mon \app{\lem[wit={A, E}]{Fils,}\rdg[wit={B, C, D}]{fils,}} à ce prix \app{\lem[wit={A, E}]{voudriez~vous}\rdg[wit={B, C, D}]{voudriez-vous}} regner~?\\}
    \vnum{47}{Mais il ne tient qu’à vous si l’honneur vous \app{\lem[wit={A, B, C, E}]{anime,}\rdg[wit={D}]{anime~;}}\\}
    \vnum{48}{De nous donner la Paix, sans le secours \app{\lem[wit={A}]{d’vn}\rdg[wit={B, C, D, E}]{d’un}} \app{\lem[wit={A, D, E}]{crime,}\rdg[wit={B, C}]{crime.}}\\}
    \vnum{49}{\app{\lem[wit={A}]{Vous pouuez-vous montrer genereux tout a fait,}\rdg[wit={B}]{Vous pouvez-vous montrer genereux tout à fait,}\rdg[wit={C}]{Vous pouvez vous montrer genereux tout à fait,}\rdg[wit={D}]{Et de vostre couroux triomphant aujourd’huy}\rdg[wit={E}]{Vous pouvez-vous montrer genereux tout a fait,}}\\}
    \vnum{50}{Contenter vostre \app{\lem[wit={A, B, C, E}]{Frere,}\rdg[wit={D}]{frere,}} \& regner \app{\lem[wit={A, B, C, E}]{en}\rdg[wit={D}]{avec}} \app{\lem[wit={A, B, C, E}]{effet.}\rdg[wit={D}]{luy.}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{ETEOCLE.}
  \begin{ekdverse}
    \vnum{51}{Appellez-vous regner \app{\lem[wit={A, E}]{luy~ceder}\rdg[wit={B, C}]{lui~ceder}\rdg[wit={D}]{partager}} ma Couronne,\\}
    \vnum{52}{\app{\lem[wit={A, B, C, E}]{Quand le sang, \& le Peuple à la fois me la donne~?}\rdg[wit={D}]{Et ceder laschement ce que mon droit me donne~?}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C, D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{53}{\app{\lem[wit={A}]{Vous sçauez bien, mon Fils, que le choix \& le sang}\rdg[wit={B, C, D}]{Vous le sçavez, mon Fils, la justice \& le sang}\rdg[wit={E}]{Vous sçavez bien, mon Fils, que le choix \& le sang}}\\}
    \vnum{54}{Luy \app{\lem[wit={A, B, D, E}]{donnent}\rdg[wit={C}]{donne}} comme à vous sa part à ce haut rang.\\}
    \vnum{55}{Oedipe en \app{\lem[wit={A}]{acheuant}\rdg[wit={B, C, D, E}]{achevant}} sa triste destinée\\}
    \vnum{56}{Ordonna que chacun regneroit son année,\\}
    \vnum{57}{Et n’ayant \app{\lem[wit={A}]{qu’vn}\rdg[wit={B, C, D, E}]{qu’un}} Estat à mettre sous vos \app{\lem[wit={A, E}]{Lois,}\rdg[wit={B, C}]{Loix,}\rdg[wit={D}]{lois,}}\\}
    \vnum{58}{\app{\lem[wit={A, B, C, E}]{Il voulut que tous deux vous en fussiez les Rois.}\rdg[wit={D}]{Voulut que tour à tour vous fussiez tous deux Rois.}}\\}
    \vnum{59}{A ces conditions vous \app{\lem[wit={A, B, C, E}]{voulustes}\rdg[wit={D}]{daignastes}} \app{\lem[wit={A, B, C, E}]{souscrire,}\rdg[wit={D}]{souscrire.}}\\}
    \vnum{60}{Le sort vous appella le premier à l’Empire,\\}
    \vnum{61}{Vous montastes au Trosne, il n’en fut point jaloux,\\}
    \vnum{62}{Et vous ne voulez pas qu’il y monte \app{\lem[wit={A, B, E}]{apres}\rdg[wit={C, D}]{aprés}} vous~?\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, E}]{ETEOCLE.}\rdg[wit={B, C, D}]{(lacune)}}}
  \begin{ekdverse}
    \vnum{63}{\app{\lem[wit={A, E}]{Il est vray, je promis, ce que voulut mon Pere,}\rdg[wit={B, C, D}]{(lacune)}}\\}
    \vnum{64}{\app{\lem[wit={A}]{Pour vn Trosne est-il rien qu’on refuse de faire~?}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{Pour un Trosne est-il rien qu’on refuse de faire~?}}\\}
    \vnum{65}{\app{\lem[wit={A}]{On promet tout, Madame, afin d’y paruenir,}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{On promet tout, Madame, afin d’y parvenir,}}\\}
    \vnum{66}{\app{\lem[wit={A, E}]{Mais on ne songe apres qu’à s’y bien maintenir.}\rdg[wit={B, C, D}]{(lacune)}}\\}
    \vnum{67}{\app{\lem[wit={A}]{I’estois alors sujet, \& dans l’obeïssance,}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{J’estois alors sujet, \& dans l’obeïssance,}}\\}
    \vnum{68}{\app{\lem[wit={A, E}]{Et je tiens aujourd’huy la supréme puissance :}\rdg[wit={B, C, D}]{(lacune)}}\\}
    \vnum{69}{\app{\lem[wit={A}]{Ce que ie fis alors ne m’est plus vne Loy,}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{Ce que je fis alors ne m’est plus une Loy,}}\\}
    \vnum{70}{\app{\lem[wit={A}]{Le deuoir d’vn Sujet n’est pas celuy d’vn Roy.}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{Le devoir d’un Sujet n’est pas celuy d’un Roy.}}\\}
    \vnum{71}{\app{\lem[wit={A, E}]{D’abord que sur sa teste il reçoit la Couronne,}\rdg[wit={B, C, D}]{(lacune)}}\\}
    \vnum{72}{\app{\lem[wit={A}]{Vn Roy sort à l’instant de sa propre personne,}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{Un Roy sort à l’instant de sa propre personne,}}\\}
    \vnum{73}{\app{\lem[wit={A}]{L’interest du public doit deuenir le sien,}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{L’interest du public doit devenir le sien,}}\\}
    \vnum{74}{\app{\lem[wit={A, E}]{Il doit tout à l’Estat, \& ne se doit plus rien.}\rdg[wit={B, C, D}]{(lacune)}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A}]{IOCASTE.}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{75}{\app{\lem[wit={A, E}]{Au moins doit-il, mon Fils, quelque chose à sa gloire,}\rdg[wit={B, C, D}]{(lacune)}}\\}
    \vnum{76}{\app{\lem[wit={A, E}]{Dont le soin ne doit pas sortir de sa memoire,}\rdg[wit={B, C, D}]{(lacune)}}\\}
    \vnum{77}{\app{\lem[wit={A}]{Et quand ce nouueau rang l’affranchiroit des Lois,}\rdg[wit={B, C, D}]{(lacune)}\rdg[wit={E}]{Et quand ce nouveau rang l’affranchiroit des Lois,}}\\}
    \vnum{78}{\app{\lem[wit={A, E}]{Au moins doit il tenir sa parole à des Rois.}\rdg[wit={B, C, D}]{(lacune)}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{ETEOCLE.}
  \begin{ekdverse}
    \vnum{79}{\app{\lem[wit={A, E}]{Polinice à ce titre auroit tort de pretendre,}\rdg[wit={B, C}]{Non, Madame, à l’Empire il ne doit plus prétendre.}\rdg[wit={D}]{Non, Madame, à l’Empire il ne doit plus prétendre~:}}\\}
    \vnum{80}{\app{\lem[wit={A}]{Thebes~sous~son~pouuoir}\rdg[wit={B, C, E}]{Thebes~sous~son~pouvoir}\rdg[wit={D}]{Thebes~à~cet~arrest}} n’a point voulu se rendre,\\}
    \vnum{81}{Et lors~que sur le Trosne il s’est voulu placer,\\}
    \vnum{82}{C’est elle \& non pas moy qui l’en a \app{\lem[wit={A, E}]{sçeû}\rdg[wit={B, C, D}]{sçeu}} \app{\lem[wit={A, B, D, E}]{chasser.}\rdg[wit={C}]{chasser,}}\\}
    \vnum{83}{Thebes \app{\lem[wit={A, E}]{doit~elle}\rdg[wit={B, C, D}]{doit-elle}} moins redouter sa puissance,\\}
    \vnum{84}{\app{\lem[wit={A, B, E}]{Apres}\rdg[wit={C, D}]{Aprés}} \app{\lem[wit={A}]{auoir}\rdg[wit={B, C, D, E}]{avoir}} six mois \app{\lem[wit={A, E}]{senty}\rdg[wit={B, C, D}]{senti}} sa violence~?\\}
    \vnum{85}{Voudroit-elle obeïr à ce Prince \app{\lem[wit={A, C, D, E}]{inhumain,}\rdg[wit={B}]{inhumain}}\\}
    \vnum{86}{Qui vient d’armer \app{\lem[wit={A, E}]{contr’-elle}\rdg[wit={B, C, D}]{contre~elle}} \& le fer \& la faim~?\\}
    \vnum{87}{Prendroit-elle pour Roy \app{\lem[wit={A}]{l’Esclaue}\rdg[wit={B, C, D, E}]{l’Esclave}} de \app{\lem[wit={A, C, D, E}]{Mycéne,}\rdg[wit={B}]{Mycéne}}\\}
    \vnum{88}{Qui pour tous les Thebains n’a plus que de la haine,\\}
    \vnum{89}{Qui s’est au Roy d’Argos indignement \app{\lem[wit={A, B, E}]{soûmis,}\rdg[wit={C, D}]{soumis,}}\\}
    \vnum{90}{Et que l’Hymen attache à nos fiers ennemis~?\\}
    \vnum{91}{Lors~que le Roy d’Argos l’a choisi pour son Gendre,\\}
    \vnum{92}{Il esperoit par luy de voir Thebes en cendre,\\}
    \vnum{93}{L’amour \app{\lem[wit={A, E}]{eût}\rdg[wit={B, C, D}]{eut}} peu de part à \app{\lem[wit={A, E}]{cét}\rdg[wit={B, C, D}]{cet}} hymen honteux,\\}
    \vnum{94}{Et la seule fureur en alluma les feux.\\}
    \vnum{95}{Thebes m’a couronné pour \app{\lem[wit={A}]{éuiter}\rdg[wit={B, C, D, E}]{éviter}} ses \app{\lem[wit={A, E}]{chaisnes~;}\rdg[wit={B}]{chaisnes.}\rdg[wit={C}]{chaisnes,}\rdg[wit={D}]{chaînes,}}\\}
    \vnum{96}{Elle s’attend par moy de voir finir ses peines,\\}
    \vnum{97}{Il la faut accuser si je manque de \app{\lem[wit={A, B, E}]{Foy,}\rdg[wit={C, D}]{foy,}}\\}
    \vnum{98}{Et je suis son Captif, \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} ne suis pas son \app{\lem[wit={A, B, D, E}]{Roy.}\rdg[wit={C}]{Roi.}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C, D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{99}{Dites, dites \app{\lem[wit={A, E}]{plutost,}\rdg[wit={B, C, D}]{plûtost,}} cœur ingrat \& farouche,\\}
    \vnum{100}{Qu’auprés du \app{\lem[wit={A, D, E}]{Diadême}\rdg[wit={B, C}]{Diadéme}} il n’est rien qui vous \app{\lem[wit={A, E}]{touche~;}\rdg[wit={B, C, D}]{touche.}}\\}
    \vnum{101}{Mais \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} me trompe \app{\lem[wit={A, B, C, E}]{encor}\rdg[wit={D}]{encor,}} ce rang ne vous plaist pas,\\}
    \vnum{102}{Et le crime tout seul a pour vous des appas.\\}
    \vnum{103}{\app{\lem[wit={A, E}]{Hé~!~bien,}\rdg[wit={B, C, D}]{He~bien~!}} puis~qu’à ce point vous en estes \app{\lem[wit={A}]{auide,}\rdg[wit={B, C, D, E}]{avide,}}\\}
    \vnum{104}{\app{\lem[wit={A}]{Ie}\rdg[wit={B, C, D, E}]{Je}} vous offre à commettre \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} double parricide,\\}
    \vnum{105}{Versez le sang \app{\lem[wit={A}]{d’vn}\rdg[wit={B, C, D, E}]{d’un}} Frere~: Et si c’est peu du sien,\\}
    \vnum{106}{\app{\lem[wit={A}]{Ie}\rdg[wit={B, C, D, E}]{Je}} vous \app{\lem[wit={A}]{inuite}\rdg[wit={B, C, D, E}]{invite}} encore à \app{\lem[wit={A, E}]{respandre}\rdg[wit={B, C, D}]{répandre}} le mien.\\}
    \vnum{107}{Vous n’aurez plus alors d’ennemis à \app{\lem[wit={A, E}]{sousmettre,}\rdg[wit={B, C}]{soûmettre,}\rdg[wit={D}]{soumettre,}}\\}
    \vnum{108}{D’obstacle à surmonter \app{\lem[wit={A, E}]{ny}\rdg[wit={B, C, D}]{ni}} de crime à commettre,\\}
    \vnum{109}{Et n’ayant plus au Trosne \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} \app{\lem[wit={A, E}]{fascheux}\rdg[wit={B, C, D}]{fâcheux}} \app{\lem[wit={A, D, E}]{concurrent,}\rdg[wit={B, C}]{concurrant,}}\\}
    \vnum{110}{De tous les Criminels vous serez le plus \app{\lem[wit={A, E}]{grand,}\rdg[wit={B, C, D}]{grand.}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{ETEOCLE.}
  \begin{ekdverse}
    \vnum{111}{Hé bien, Madame, hé bien, il faut vous satisfaire,\\}
    \vnum{112}{Il faut sortir du Trosne \& couronner mon \app{\lem[wit={A, B, E}]{frere,}\rdg[wit={C}]{Frere,}\rdg[wit={D}]{Frere~:}}\\}
    \vnum{113}{Il faut pour seconder vostre injuste projet,\\}
    \vnum{114}{De son Roy que j’estois \app{\lem[wit={A}]{deuenir}\rdg[wit={B, C, D, E}]{devenir}} son \app{\lem[wit={A, E}]{sujet.}\rdg[wit={B, C}]{Sujet.}\rdg[wit={D}]{Sujet~;}}\\}
    \vnum{115}{Et pour vous \app{\lem[wit={A}]{éleuer}\rdg[wit={B, C, D, E}]{élever}} au comble de la \app{\lem[wit={A, D, E}]{joye,}\rdg[wit={B, C}]{joye~;}}\\}
    \vnum{116}{Il faut à sa fureur que \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} me \app{\lem[wit={A}]{liure}\rdg[wit={B, C, D, E}]{livre}} en proye,\\}
    \vnum{117.1}{Il faut par mon \app{\lem[wit={A, B, E}]{trépas….}\rdg[wit={C}]{trépas…}\rdg[wit={D}]{trépas.…}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{\app{\lem[wit={A, B}]{IOCASTE.}\rdg[wit={C, D, E}]{JOCASTE.}}}
  \begin{ekdverse}
    \vnum{117.2}{Ah Ciel~! quelle \app{\lem[wit={A, E}]{rigueur,}\rdg[wit={B, C, D}]{rigueur~!}}\\}
    \vnum{118}{Que vous penetrez mal dans le \app{\lem[wit={A, B, C, E}]{fonds}\rdg[wit={D}]{fond}} de mon cœur~!\\}
    \vnum{119}{\app{\lem[wit={A}]{Ie}\rdg[wit={B, C, D, E}]{Je}} ne demande pas que vous quittiez \app{\lem[wit={A, E}]{l’Empire,}\rdg[wit={B, C, D}]{l’Empire.}}\\}
    \vnum{120}{Regnez \app{\lem[wit={A}]{tousiours,}\rdg[wit={B, C, D}]{toûjours,}\rdg[wit={E}]{tousjours,}} mon Fils, c’est ce que \app{\lem[wit={A, E}]{ie}\rdg[wit={B, C, D}]{je}} desire.\\}
    \vnum{121}{Mais si tant de \app{\lem[wit={A, D, E}]{malheurs}\rdg[wit={B, C}]{mal-heurs}} vous touchent de pitié,\\}
    \vnum{122}{Si pour moy vostre cœur garde quelque \app{\lem[wit={A, B, C, E}]{amitié~;}\rdg[wit={D}]{amitié,}}\\}
    \vnum{123}{Et si vous prenez soin de vostre gloire \app{\lem[wit={A, B, C, E}]{mesme,}\rdg[wit={D}]{même,}}\\}
    \vnum{124}{Associez un Frere à \app{\lem[wit={A, E}]{cét}\rdg[wit={B, C, D}]{cet}} honneur \app{\lem[wit={A, B, C, E}]{supréme~;}\rdg[wit={D}]{suprême~;}}\\}
    \vnum{125}{Ce n’est \app{\lem[wit={A}]{qu’vn}\rdg[wit={B, C, D, E}]{qu’un}} vain éclat qu’il \app{\lem[wit={A}]{receura}\rdg[wit={B, C, D, E}]{recevra}} de vous,\\}
    \vnum{126}{Vostre regne en sera plus puissant \& plus doux.\\}
    \vnum{127}{Les Peuples admirant cette vertu sublime,\\}
    \vnum{128}{Voudront \app{\lem[wit={A}]{tousiours}\rdg[wit={B, C, D}]{toûjours}\rdg[wit={E}]{tousjours}} pour Prince \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} Roy si magnanime,\\}
    \vnum{129}{Et \app{\lem[wit={A, B, E}]{cét}\rdg[wit={C, D}]{cet}} illustre effort, loin d’affoiblir vos droits,\\}
    \vnum{130}{Vous rendra le plus juste \& le plus grand des Rois.\\}
    \vnum{131}{Ou s’il faut que mes vœux vous \app{\lem[wit={A}]{treuuent}\rdg[wit={B, C, D}]{trouvent}\rdg[wit={E}]{treuvent}} inflexible,\\}
    \vnum{132}{Si la Paix à ce prix vous paroist impossible,\\}
    \vnum{133}{Et \app{\lem[wit={A, E}]{que}\rdg[wit={B, C, D}]{si}} le \app{\lem[wit={A, E}]{Diadéme}\rdg[wit={B, C, D}]{Diadême}} \app{\lem[wit={A, E}]{ait}\rdg[wit={B, C, D}]{a}} pour vous tant d’attraits,\\}
    \vnum{134}{Au moins \app{\lem[wit={A, C, D, E}]{consolez-moy}\rdg[wit={B}]{consolez~moy}} de quelque heure de \app{\lem[wit={A, E}]{Paix,}\rdg[wit={B, C, D}]{paix.}}\\}
    \vnum{135}{\app{\lem[wit={A, B, C, E}]{Accordez quelque tréve à ma douleur amere,}\rdg[wit={D}]{Accordez cette grace aux larmes d’une Mere.}}\\}
    \vnum{136}{Et cependant, mon Fils, \app{\lem[wit={A, B, D, E}]{j’iray}\rdg[wit={C}]{j’irai}} voir vostre Frere,\\}
    \vnum{137}{La pitié dans son ame aura \app{\lem[wit={A, E}]{peut~estre}\rdg[wit={B, C, D}]{peut-estre}} lieu,\\}
    \vnum{138}{Ou du moins pour jamais \app{\lem[wit={A, B, D, E}]{j’iray}\rdg[wit={C}]{j’irai}} \app{\lem[wit={A, D, E}]{luy}\rdg[wit={B, C}]{lui}} dire \app{\lem[wit={A, E}]{Adieu.}\rdg[wit={B, C, D}]{adieu.}}\\}
    \vnum{139}{Dés ce mesme moment permettez que \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} sorte,\\}
    \vnum{140}{\app{\lem[wit={A}]{I’iray}\rdg[wit={B, D, E}]{J’iray}\rdg[wit={C}]{J’irai}} jusqu’à sa tente, \& \app{\lem[wit={A, B, D, E}]{j’iray}\rdg[wit={C}]{j’irai}} sans escorte,\\}
    \vnum{141}{\app{\lem[wit={A}]{Dans cette occasion rien ne peut m’émouuoir.}\rdg[wit={B, C, E}]{Dans cette occasion rien ne peut m’émouvoir.}\rdg[wit={D}]{Par mes justes soupirs j’espere l’émouvoir.}}\\}
  \end{ekdverse}
\end{speech}
\begin{speech}
  \speaker{ETEOCLE.}
  \begin{ekdverse}
    \vnum{142}{Madame, sans sortir vous le \app{\lem[wit={A}]{pouuez}\rdg[wit={B, C, D, E}]{pouvez}} \app{\lem[wit={A, E}]{bien~voir.}\rdg[wit={B, C}]{revoir.}\rdg[wit={D}]{revoir~;}}\\}
    \vnum{143}{Et si cette \app{\lem[wit={A}]{entreueuë}\rdg[wit={B, C, D, E}]{entreveuë}} \app{\lem[wit={A, E}]{à}\rdg[wit={B, C, D}]{a}} pour vous tant de charmes,\\}
    \vnum{144}{Il ne tiendra qu’à \app{\lem[wit={A, B, D, E}]{luy}\rdg[wit={C}]{lui}} de suspendre nos \app{\lem[wit={A, B, C, E}]{armes,}\rdg[wit={D}]{armes~:}}\\}
    \vnum{145}{Vous \app{\lem[wit={A}]{pouuez}\rdg[wit={B, C, D, E}]{pouvez}} dés cette heure accomplir vos souhaits,\\}
    \vnum{146}{Et le faire venir jusques dans ce Palais.\\}
    \vnum{147}{\app{\lem[wit={A}]{Ie~feray~plus~encore,}\rdg[wit={B}]{Je~feray~plus~encor,}\rdg[wit={C}]{Je~ferai~plus~encore,}\rdg[wit={D}]{J’iray~plus~loin~encore,}\rdg[wit={E}]{Je~feray~plus~encore,}} \& pour faire \app{\lem[wit={A, E}]{connoistre,}\rdg[wit={B, C, D}]{connaistre,}}\\}
    \vnum{148}{Qu’il a tort en effet de me nommer \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} traistre,\\}
    \vnum{149}{Et que \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} ne suis pas \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} tyran odieux,\\}
    \vnum{150}{Que l’on fasse parler \& le \app{\lem[wit={A, B, E}]{Peuple}\rdg[wit={C, D}]{peuple}} \& les \app{\lem[wit={A, E}]{Dieux.}\rdg[wit={B, C, D}]{dieux.}}\\}
    \vnum{151}{Si le \app{\lem[wit={A, B, C, E}]{Peuple}\rdg[wit={D}]{peuple}} \app{\lem[wit={A, B, C, E}]{le~veut,}\rdg[wit={D}]{y~consent,}} \app{\lem[wit={A}]{ie}\rdg[wit={B, C, D, E}]{je}} \app{\lem[wit={A, B, D, E}]{luy}\rdg[wit={C}]{lui}} cede ma \app{\lem[wit={A, E}]{place,}\rdg[wit={B, C, D}]{place.}}\\}
    \vnum{152}{Mais qu’il se rende \app{\lem[wit={A, E}]{aussi}\rdg[wit={B}]{aussy}\rdg[wit={C, D}]{enfin}} si le \app{\lem[wit={A, B, C, E}]{Peuple}\rdg[wit={D}]{peuple}} le chasse,\\}
    \vnum{153}{\app{\lem[wit={A}]{Ie}\rdg[wit={B, C, D, E}]{Je}} ne force personne, \& j’engage ma \app{\lem[wit={A, B, D, E}]{foy}\rdg[wit={C}]{foi}}\\}
    \vnum{154}{De laisser aux Thebains à se choisir \app{\lem[wit={A}]{vn}\rdg[wit={B, C, D, E}]{un}} \app{\lem[wit={A, B, D, E}]{Roy.}\rdg[wit={C}]{Roi.}}\\}
  \end{ekdverse}
\end{speech}
